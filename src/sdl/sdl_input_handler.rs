...
impl InputHandler for SDLInputHandler {
    fn capture(&mut self) -> Vec<InputState> {
        ...
        if SDLInputHandler::is_pressed(&state, &[Scancode::Up, Scancode::W]) { input.push(InputState::Up) }
        if SDLInputHandler::is_pressed(&state, &[Scancode::Down, Scancode::S]) { input.push(InputState::Down) }
        if SDLInputHandler::is_pressed(&state, &[Scancode::Left, Scancode::A]) { input.push(InputState::Left) }
        if SDLInputHandler::is_pressed(&state, &[Scancode::Right, Scancode::D]) { input.push(InputState::Right) }
        if SDLInputHandler::is_pressed(&state, &[Scancode::Space]) { input.push(InputState::Shoot) }
        input
    }
}
